...
def create_mv_places_brand():
    ...
    try:
        ...
        # exclusive lock for the DROP + RENAME at the very end. A DROP first
        # would hold that lock for the whole build — every request hanging on
        # it, no maintenance page, since this step opens no data_imports row.
        # One transaction: a failure leaves the old view exactly as it was.
        with conn.cursor() as cur:
            ...
            cur.execute(
                "ALTER MATERIALIZED VIEW mv_places_brand_new RENAME TO mv_places_brand;"
            )
        conn.commit()
