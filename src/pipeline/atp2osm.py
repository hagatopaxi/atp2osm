...
def mv_places_brand_sql(name: str = "mv_places_brand") -> sql.Composed:
    ...
    return sql.SQL("""
        ...
        CROSS JOIN LATERAL (VALUES {waves}) AS w(wave, matches)
        WHERE w.matches
        GROUP BY atp_brand_wikidata, subdivision_code, w.wave
    """).format(name=sql.Identifier(name), matched=matched_poi_sql(), waves=waves_lateral_sql())


def mv_places_spider_sql(name: str = "mv_places_spider") -> sql.Composed:
    ...
    return sql.SQL("""
        ...
        FROM ({matched}) matched
        GROUP BY spider_id
    """).format(name=sql.Identifier(name), matched=matched_poi_sql())
...
def create_mv_places_brand() -> None:
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(mv_places_brand_sql("mv_places_brand_new"))
            ...
            cur.execute(mv_places_spider_sql("mv_places_spider_new"))
