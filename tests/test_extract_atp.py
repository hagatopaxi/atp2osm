...
def test_the_spiders_of_the_country_are_extracted_flat(workdir):
    """ATP nests them under output/; the next steps read one directory."""
    zipped(workdir / "output.zip", [
    ...
    assert sorted(p.name for p in (workdir / "geojson").glob("*.geojson")) == [
        "babylone.geojson", "babylone_fr.geojson", "babylone_mq.geojson",
    ]


def test_a_previous_extraction_is_replaced(workdir):
...
def test_a_zip_without_a_spider_of_ours_is_an_error(workdir):
    zipped(workdir / "output.zip", ["output/babylone_de.geojson"])
    with pytest.raises(FileNotFoundError):
        atp.extract_atp()


def test_no_zip_means_nothing_was_downloaded(workdir):
    atp.extract_atp()
