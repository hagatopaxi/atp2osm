...
def test_fills_the_remaining_room_with_the_biggest_that_fits() -> None:
    # 180 leaves 20: takes 20, not 15
    assert pack_subdivisions({"75": 180, "69": 20, "13": 15}, 200)[0] == ["69", "75"]


def test_no_batch_exceeds_max_and_all_subdivisions_are_used_once() -> None:
...
def test_oversized_departement_gets_its_own_batch() -> None:
...
def test_compose_batch_skips_blocked() -> None:
    counts = {"69": 120, "59": 90, "33": 60}
    assert compose_batch(counts, blocked={"69"}, max_size=200) == ["33", "59"]


def test_compose_batch_first_batch_only() -> None:
    counts = {"69": 150, "59": 150, "33": 150}
    assert compose_batch(counts, blocked=set(), max_size=200) == ["33"]


def test_compose_batch_all_blocked() -> None:
    assert compose_batch({"69": 10}, blocked={"69"}, max_size=200) == []


def _changes(**by_subdivision: int) -> list[Change]:
    return [
        make_change(subdivision_code=d, subdivision_name=None)
        for d, n in by_subdivision.items()
        for _ in range(n)
    ]


def test_select_batch_keeps_only_the_first_batch_and_describes_it() -> None:
...
def test_select_batch_never_cuts_a_departement_in_two() -> None:
...
def test_select_batch_truncates_an_oversized_subdivision() -> None:
...
def test_select_batch_returns_nothing_when_everything_is_blocked() -> None:
    assert select_batch(_changes(d69=3), blocked={"d69"}, max_size=10) == []


def test_select_batch_never_exceeds_max_size() -> None:
...
def test_bulk_upload_refuses_an_oversized_batch() -> None:
    ...
    with pytest.raises(ValueError, match="refusing to upload"):
        BulkUpload([make_change()] * (BATCH_MAX_SIZE + 1), session=OAuth2Session())


def test_count_by_subdivision() -> None:
    changes = [make_change(subdivision_code=d) for d in ("69", "59", "69")]
