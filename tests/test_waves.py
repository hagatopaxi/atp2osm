...
def match(tags, modifiable, **extra):
    base = {
        "osm_id": 1,
        "version": 1,
        "node_type": "node",
...
def test_wave_2_replaces_the_value_the_object_carries():
    res = match({"website": "https://babylone.fr"}, {"website": "https://www.babylone.fr"})
    assert res["tag"] == {"website": "https://www.babylone.fr"}


def test_wave_2_writes_on_the_key_that_holds_the_value():
    """An object tagged contact:phone keeps its own spelling, it grows no second one."""
    res = match({"contact:phone": "01 23 45 67 89"}, {"phone": "+33 8 20 33 22 11"})
    assert res["tag"] == {"contact:phone": "08 20 33 22 11"}


def test_wave_2_rewrites_every_writing_present():
...
def test_every_wave_has_its_own_batch_size():
...
def integrate(conn, subdivision, wave):
...
def test_a_brand_starts_on_its_first_wave(brand_waves):
    with brand_waves.cursor(row_factory=dict_row) as cur:
        assert current_wave(cur, "Q1").number == 1


def test_a_wave_1_cooldown_does_not_block_wave_2(brand_waves):
...
def test_the_next_wave_comes_only_once_the_first_is_done(brand_waves):
    integrate(brand_waves, "75", wave=1)
    with brand_waves.cursor(row_factory=dict_row) as cur:
