...
def _nsi(categories: dict[str, Json]) -> Json:
    return {"nsi": categories}


def _category(*items: Json) -> Json:
    return {"properties": {}, "items": list(items), "templates": []}


def _item(tags: dict[str, str], location_set: Json, **extra: Any) -> Json:  # noqa: ANN401 — whatever NSI holds
    return {
        "displayName": tags.get("brand", ""),
        "id": "x",
        "tags": tags,
...
def test_keeps_only_the_french_variant() -> None:
    ...
    nsi = _nsi(
        {
            "brands/amenity/fast_food": _category(
                _item(
                    {"brand": "McDonald's", "brand:wikidata": "Q38076", "amenity": "fast_food"},
                ...
                _item(
                    {"brand": "マクドナルド", "brand:wikidata": "Q38076", "amenity": "fast_food"},
                    {"include": ["jp"]},
                ),
            )
        }
...
def test_worldwide_items_cover_france() -> None:
    nsi = _nsi(
        {
            "brands/shop/clothes": _category(
                _item(
                    {"brand": "Zara", "brand:wikidata": "Q147662", "shop": "clothes"},
                    {"include": ["001"]},
                ),
                _item(
                    {"brand": "Uniqlo", "brand:wikidata": "Q26070", "shop": "clothes"},
                    {"include": ["jp"]},
                ),
            )
        }
...
def test_same_qid_in_different_categories_all_survive() -> None:
    ...
    nsi = _nsi(
        {
            ...
            "brands/amenity/fuel": _category(
                _item(
                    {"brand": "E.Leclerc", "brand:wikidata": "Q1273376", "amenity": "fuel"},
                    {"include": ["fr"]},
                )
            ),
        }
...
def test_same_qid_and_category_agreeing_on_tags_are_kept() -> None:
    ...
    nsi = _nsi(
        {
            "brands/shop/supermarket": _category(
                _item(
                    {"brand": "E.Leclerc", "brand:wikidata": "Q1273376", "shop": "supermarket"},
                    {"include": ["fr"]},
                ),
                _item(
                    {
...
def test_same_qid_and_category_disagreeing_on_tags_cancel_each_other() -> None:
    ...
    nsi = _nsi(
        {
            "brands/shop/supermarket": _category(
                _item(
                    {"brand": "Intermarché", "brand:wikidata": "Q3153200", "shop": "supermarket"},
                    {"include": ["001"]},
                ),
                _item(
                    {
                ...
                _item(
                    {"brand": "Lidl", "brand:wikidata": "Q151954", "shop": "supermarket"},
                    {"include": ["fr"]},
                ),
            )
        }
...
def test_only_writable_tags_are_kept() -> None:
...
def test_ignores_transit_and_items_without_qid() -> None:
    nsi = _nsi(
        {
            "transit/route/tram": _category(
                _item(
                    {"brand": "T1", "brand:wikidata": "Q42", "route": "tram"}, {"include": ["fr"]}
                )
            ),
...
def test_ignores_categories_generic_lua_drops() -> None:
    ...
    nsi = _nsi(
        {
            "brands/advertising/totem": _category(
                _item(
                    {"brand": "Intermarché", "brand:wikidata": "Q3153200", "advertising": "totem"},
                    {"include": ["fr"]},
                )
            ),
...
def test_metropolitan_france_code_counts_as_french() -> None:
    ...
    nsi = _nsi(
        {
            "brands/shop/jewelry": _category(
                _item(
                    {"brand": "Manège à Bijoux", "brand:wikidata": "Q1273376", "shop": "jewelry"},
                    {"include": ["fx"]},
                ),
            )
        }
...
def test_overseas_only_brands_are_kept() -> None:
    ...
    nsi = _nsi(
        {
            "brands/amenity/fuel": _category(
                _item(
                    {"brand": "Sol", "brand:wikidata": "Q3488375", "amenity": "fuel"},
...
def test_stamp_changes_with_the_deployed_revision(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    before = nsi.stamp("8.0.20260729")
    monkeypatch.setattr(nsi, "app_version", lambda: "Gamma-decafe")
    assert nsi.stamp("8.0.20260729") != before


def test_stamp_carries_the_published_version(monkeypatch: pytest.MonkeyPatch) -> None:
    from src.pipeline import nsi

    assert nsi.stamp("8.0.20260729").startswith("8.0.20260729+")


def test_a_containing_code_counts_on_both_sides() -> None:
    """An item scoped to the world applies here; the world minus Europe does not."""
    from src.pipeline.nsi import is_country

    assert is_country({"include": ["001"]})
    assert is_country({"include": ["150"], "exclude": ["de"]})
    assert not is_country({"include": ["001"], "exclude": ["150"]})
    assert not is_country({"include": ["001"], "exclude": ["fr"]})
    assert not is_country({"include": ["de", "be"]})
