...
@pytest.fixture
def conn(
    migrated_conn: Connection, test_db: Database, monkeypatch: pytest.MonkeyPatch
) -> Connection:
    """The pipeline opens connections of its own: point them here too."""
    monkeypatch.setattr(dag, "connect", lambda: psycopg.connect(test_db.conninfo))
    return migrated_conn


def _latest(conn: Connection, import_type: str) -> tuple[str, str | None]:
    return one(
        conn.execute(
            "SELECT status, comment FROM data_imports WHERE type=%s ORDER BY created_at DESC LIMIT 1",
            (import_type,),
        ).fetchone()
    )


def test_record_import_resolves_the_open_row_instead_of_stacking_one(conn: Connection) -> None:
    ...
    count = one(conn.execute("SELECT COUNT(*) FROM data_imports WHERE type='osm'").fetchone())
    assert count[0] == 1


def test_a_failing_step_closes_the_open_row_with_its_trace(conn: Connection) -> None:
    start_import(conn, "osm")

    record_failure("osm-whatever", RuntimeError("boom"))

    row = conn.execute("SELECT status, comment FROM data_imports WHERE type='osm'").fetchall()
    assert len(row) == 1
    assert row[0][0] == "pending"
    assert "boom" in row[0][1]


def test_a_failed_run_leaves_the_guards_on_the_last_resolved_stamp(conn: Connection) -> None:
...
def test_a_relaunch_supersedes_the_row_a_crashed_run_left_behind(conn: Connection) -> None:
...
def test_an_unreachable_source_is_recorded_skipped_on_its_last_date(conn: Connection) -> None:
    ...
    record_failure("osm-download", SourceUnavailableError("Geofabrik"))

    date, status, comment = one(
        conn.execute(
...
def test_a_shared_step_failing_lands_on_the_pipeline_row(conn: Connection) -> None:
