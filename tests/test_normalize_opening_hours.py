...
# The SQL function, called from Python: a value and whether the days closed
# are written, the normalized value.
Normalize = Callable[..., str | None]  # (value, *, with_off=False)
...
def normalize(test_db: Database) -> Iterator[Normalize]:
    with psycopg.connect(test_db.conninfo) as conn:
        ...
        def _normalize(value: str | None, *, with_off: bool = False) -> str | None:
            return one(
                conn.execute("SELECT normalize_opening_hours(%s, %s)", (value, with_off)).fetchone()
            )[0]
    ...
    [
        ...
        (
            ...
            [
                ...
                "Mo-Sa 09:00-19:00; Su off",
                "Mo-Sa  09:00 - 19:00",
                "Mo-Sa 09:00-19:00; PH off",
                "Mo-Sa 09:00-19:00;",
                "Mo-Sa 09:00-19:00 ; Su off ;",
            ],
        ),
        ...
        (
            "Mo-Sa 09:00-21:30",
            ["Mo-Sa,PH 09:00-21:30", "PH,Mo-Sa 09:00-21:30", "Mo-Sa 09:00-21:30; PH 10:00-12:00"],
        ),
        (
            "Mo-Sa 09:00-19:00; Su 10:00-12:00",
            ["Mo-Sa 09:00-19:00; Su,PH 10:00-12:00", "Mo-Sa 09:00-19:00; PH,Su 10:00-12:00"],
        ),
    ],
)
def test_one_writing_per_week(normalize: Normalize, canonical: str, spellings: list[str]) -> None:
    assert normalize(canonical) == canonical
    for spelling in spellings:
        assert normalize(spelling) == canonical, spelling


def test_the_days_declared_closed_are_written_on_request(normalize: Normalize) -> None:
    ...
    assert (
        normalize("Mo,We,Fr 09:00-19:00; Tu,Th off; Sa-Su closed", with_off=True)
        == "Mo,We,Fr 09:00-19:00; Tu,Th,Sa-Su off"
    )
    ...
    assert (
        normalize("Mo-Su off; Mo-Fr 09:00-19:00; PH off", with_off=True)
        == "Mo-Fr 09:00-19:00; Sa-Su off"
    )
    # A midnight split closes nothing: ATP unmarks the day it spills into.
    assert (
        normalize("Fr 22:00-24:00; Sa 00:00-02:00; Su closed", with_off=True)
        == "Fr 22:00-02:00; Su off"
    )


def test_a_rule_after_a_comma_adds_to_the_days_it_names(normalize: Normalize) -> None:
    """The wiki's own example: Tu-Fr open morning and evening both."""
    assert (
        normalize("Mo-Fr 08:00-12:00, Tu-Sa 18:00-22:00")
        == "Mo 08:00-12:00; Tu-Fr 08:00-12:00,18:00-22:00; Sa 18:00-22:00"
    )
...
def test_overlapping_ranges_are_one(normalize: Normalize) -> None:
...
def test_an_explicit_open_says_nothing(normalize: Normalize) -> None:
...
def test_a_later_rule_overrides_the_days_it_names(normalize: Normalize) -> None:
    assert (
        normalize("Mo-Sa 07:30-21:00; Fr,Su 07:30-21:30")
        == "Mo-Th,Sa 07:30-21:00; Fr,Su 07:30-21:30"
    )
    assert normalize("Mo-Sa 09:00-19:00; Th off") == "Mo-We,Fr-Sa 09:00-19:00"


def test_a_full_day_is_not_joined_to_the_next(normalize: Normalize) -> None:
    assert normalize("Mo 00:00-24:00; Tu 00:00-02:00") == "Mo 00:00-24:00; Tu 00:00-02:00"


def test_different_hours_stay_different(normalize: Normalize) -> None:
    ...
    assert normalize("Mo-Fr 08:00-18:00") != normalize(
        "Mo-Fr 08:00-12:00,12:00-18:00; Sa 09:00-12:00"
    )


def test_a_week_of_nothing_is_nothing(normalize: Normalize) -> None:
...
def test_rules_that_say_nothing_about_the_week_are_left_out(normalize: Normalize) -> None:
...
def test_a_week_it_cannot_read_is_nobodys_to_write(normalize: Normalize, value: str) -> None:
...
@pytest.mark.parametrize("value", UNREADABLE)
def test_merge_refuses_what_the_comparison_could_not_read(value: str) -> None:
    ...
    """
    with pytest.raises(ValueError, match="not read as a week"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Mo-Fr 08:00-18:00 || Mo-Su 06:00-23:00 open",
            "Mo-Sa 08:00-19:00 || Mo-Su 06:00-23:00 open",
        ),
...
def test_merge_keeps_what_was_left_out(old: str, written: str) -> None:
...
@pytest.mark.parametrize(
    "atp",
    ["Mo-Sa 08:00-19:00; Su closed", "Mo-Fr 09:00-24:00; Sa 00:00-01:00", "Mo-Su 00:00-24:00"],
)
def test_what_is_written_reads_back_as_atps_week(normalize: Normalize, old: str, atp: str) -> None:
    ...
    """
    written = merge_opening_hours(old, one(normalize(atp, with_off=True)))
