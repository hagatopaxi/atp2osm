...
def _no_user_lookup(monkeypatch):
    ...
    monkeypatch.setattr(todo, "fetch_osm_users", lambda ids: {uid: f"user-{uid}" for uid in ids})
...
def add(conn, name, wikidata=None, user=42, estimation=None):
...
def test_the_biggest_brands_come_first(web_app, migrated_conn, rendered):
...
def test_the_list_can_be_sorted_on_a_column(web_app, migrated_conn, rendered):
...
def test_brands_already_in_atp_are_hidden_unless_asked(web_app, migrated_conn, rendered):
    add(migrated_conn, "Known", "Q1", estimation=10)
    add(migrated_conn, "Missing", "Q2", estimation=10)
    with migrated_conn.cursor() as cur:
        cur.execute("INSERT INTO atp_places (id, spider_id, brand_wikidata, brand)"
...
def test_a_brand_known_to_atp_by_its_name_only_is_hidden_too(web_app, migrated_conn, rendered):
...
def test_the_search_and_the_user_filter_narrow_the_list(web_app, migrated_conn, rendered):
...
def test_the_signed_in_contributor_is_told_apart(contributor, migrated_conn, rendered):
    add(migrated_conn, "Babylone", "Q1", user=42)
...
def test_check_finds_the_brand_by_its_code_first(web_app, migrated_conn):
    ...
    assert [m["brand_wikidata"] for m in res.json["matches"]] == ["Q2"]


def test_check_falls_back_on_the_name(web_app, migrated_conn):
    add(migrated_conn, "Babylone", "Q1")
    add(migrated_conn, "Babylone Kids", "Q2")
    add(migrated_conn, "Other", "Q3")
    res = web_app.test_client().get("/todo/check?wikidata=Q999&name=babyl")
    assert sorted(m["brand_wikidata"] for m in res.json["matches"]) == ["Q1", "Q2"]


def test_check_with_nothing_to_look_for_finds_nothing(web_app, migrated_conn):
    add(migrated_conn, "Babylone", "Q1")
...
def test_a_contributor_adds_a_brand(contributor, migrated_conn):
    res = contributor.post("/todo", json={"brand_wikidata": " Q1 ", "brand_name": " Babylone ", "estimation": "120"})
    assert res.status_code == 201
    (row,) = rows(migrated_conn)
    assert (row["brand_wikidata"], row["brand_name"], row["estimation"], row["osm_user_id"]) == ("Q1", "Babylone", 120, 42)


def test_a_brand_without_a_code_is_accepted(contributor, migrated_conn):
    assert contributor.post("/todo", json={"brand_name": "Babylone", "brand_wikidata": ""}).status_code == 201
    assert rows(migrated_conn)[0]["brand_wikidata"] is None


def test_the_name_is_required(contributor, migrated_conn):
...
def test_the_estimation_must_be_a_number(contributor, migrated_conn):
    res = contributor.post("/todo", json={"brand_name": "Babylone", "estimation": "many"})
    assert res.status_code == 400
    assert rows(migrated_conn) == []


def test_the_same_code_twice_is_a_conflict(contributor, migrated_conn):
    add(migrated_conn, "Babylone", "Q1")
...
def test_an_anonymous_visitor_cannot_add(web_app, migrated_conn):
...
def test_an_entry_is_updated_and_signed(contributor, migrated_conn):
    entry_id = add(migrated_conn, "Babylone", "Q1", user=43)
    res = contributor.put(f"/todo/{entry_id}", json={"brand_wikidata": "Q1", "brand_name": "Babylone Paris", "estimation": 50})
...
def test_updating_an_unknown_entry_is_not_found(contributor, migrated_conn):
    assert contributor.put("/todo/999", json={"brand_name": "x"}).status_code == 404


def test_updating_onto_another_entry_code_is_a_conflict(contributor, migrated_conn):
    add(migrated_conn, "Babylone", "Q1")
...
def test_an_update_needs_a_name_too(contributor, migrated_conn):
...
def test_an_author_deletes_their_own_entry(contributor, migrated_conn):
    entry_id = add(migrated_conn, "Babylone", "Q1", user=42)
    assert contributor.delete(f"/todo/{entry_id}").status_code == 204
    assert rows(migrated_conn) == []


def test_nobody_deletes_someone_else_s_entry(contributor, migrated_conn):
    entry_id = add(migrated_conn, "Babylone", "Q1", user=43)
    assert contributor.delete(f"/todo/{entry_id}").status_code == 403
    assert len(rows(migrated_conn)) == 1


def test_deleting_an_unknown_entry_is_not_found(contributor, migrated_conn):
    assert contributor.delete("/todo/999").status_code == 404


def test_an_anonymous_visitor_cannot_delete(web_app, migrated_conn):
    entry_id = add(migrated_conn, "Babylone", "Q1")
